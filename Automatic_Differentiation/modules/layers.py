...
from numba.np.linalg import intp_t
...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if not self.training:
            ...
            output = self.norm_input * self.weight + self.bias if self.affine else self.norm_input
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            grad_var = np.sum(grad_norm_input * (input - self.mean) * -0.5 * self.inv_sqrt_var ** 3, axis=0)
            grad_mean = np.sum(grad_norm_input * -self.inv_sqrt_var, axis=0) + grad_var * np.mean(
                -2.0 * (input - self.mean), axis=0)
            grad_input = (grad_norm_input * self.inv_sqrt_var) + (grad_var * 2.0 * (input - self.mean) / batch_size) + (grad_mean / batch_size)
        else:
            grad_input = (grad_norm_input / np.sqrt(self.running_var + self.eps))
    ...
    def update_grad_parameters(self, input: np.ndarray, grad_output: np.ndarray):
        ...
        if self.affine:
            ...
            self.grad_weight += np.sum(grad_output * norm_input, axis=0)
            self.grad_bias += np.sum(grad_output, axis=0)
...
class Dropout(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        if self.training:
            return grad_output / (1 - self.p) * self.mask
        else:
            return grad_output
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        current_input = input
        for module in self.modules:
            current_input = module.compute_output(current_input)
        ...
        for i in range(len(self.modules) - 1, -1, -1):
            ...
            prev_grad = grad
            grad = module.compute_grad_input(activations[i], grad)
            module.update_grad_parameters(activations[i], prev_grad)
            j += 1
